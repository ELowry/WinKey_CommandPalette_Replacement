param(
	[Parameter(HelpMessage = "Action to perform: install, uninstall, startup-enable, startup-disable, status, logs, start, stop, update, silent-update, toggle-auto-update, or menu")]
	[ValidateSet("install", "uninstall", "startup-enable", "startup-disable", "status", "logs", "start", "stop", "update", "silent-update", "toggle-auto-update", "menu")]
	[string]$Action = "menu"
)
...
function Show-Menu {
	...
	$autoUpdateStatus = if (Get-AutoUpdateEnabled) { "ON" } else { "OFF" }
	Write-ColoredOutput "4. Toggle auto-update on startup (currently: $autoUpdateStatus)" "White"
	Write-ColoredOutput ""
	Write-ColoredOutput "🎮 Application Control:" $ColorSuccess
	Write-ColoredOutput "5. Start $AppName" "White"
	Write-ColoredOutput "6. Stop $AppName" "White"
	Write-ColoredOutput "7. Configure triggered shortcut" "White"

	Write-ColoredOutput ""
	Write-ColoredOutput "⚡ Startup Configuration:" $ColorWarning
	Write-ColoredOutput "8. Enable startup (run when Windows starts)" "White"
	Write-ColoredOutput "9. Disable startup" "White"
	Write-ColoredOutput ""
	Write-ColoredOutput "📊 Status & Information:" $ColorPrompt
	Write-ColoredOutput "10. Check status" "White"
	Write-ColoredOutput "11. View startup logs" "White"
	Write-ColoredOutput ""
	Write-ColoredOutput "12. Exit" "Gray"
	Write-ColoredOutput ""
	...
	switch ($choice) {
		...
		"5" { Start-Application }
		"6" { Stop-Application }
		"7" { Configure-Shortcut -InstallPath $InstallPath -ExeName $ExeName }
		"8" { Enable-Startup }
		"9" { Disable-Startup }
		"10" { Show-Status }
		"11" { Show-StartupLogs }
...
function Invoke-UpdateProcess {
	...
	$tempZip = Join-Path $env:TEMP "WinKeyRemapper-Update.zip"
	$tempExtractDir = Join-Path $env:TEMP "WinKeyRemapper-Extract"
	...
	# Write release tag directly into the extract folder so it gets copied over
	if (-not [string]::IsNullOrWhiteSpace($Version) -and $Version -ne "Unknown") {
		$Version | Out-File -FilePath (Join-Path $tempExtractDir "release-tag.txt") -Encoding UTF8 -Force
	}
	
	Remove-Item $tempZip -Force -ErrorAction SilentlyContinue

	# 3. Generate Update Helper Batch Script
	$helperPath = Join-Path $env:TEMP "winkey-update-helper.bat"
	$helperContent = @"
@echo off
REM Wait 3 seconds for PowerShell and Startup.bat to release their file locks
timeout /t 3 /nobreak > nul

REM Ensure the application is dead
taskkill /F /IM "$ExeName" > nul 2>&1

REM Use Robocopy to Mirror (/MIR) the new files. 
REM This updates files AND deletes orphans, but /XF configs
robocopy "%~1" "%~2" /MIR /XF shortcut.json auto-update.enabled last-update-check.txt winkey-startup.log
...
del "%~f0"
"@
	$helperContent | Out-File -FilePath $helperPath -Encoding ASCII -Force
	
	# 4. Handoff & Exit
	if ($IsSilent) {
		Start-Process -FilePath $helperPath -ArgumentList "`"$tempExtractDir`" `"$InstallPath`"" -WindowStyle Hidden
		# Fire specific exit code 99 to tell Startup.bat to halt!
		exit 99 
	} else {
		Write-ColoredOutput "✅ Update downloaded. Handing off to updater to swap files." $ColorSuccess
		Write-ColoredOutput "This window will close, and the app will restart momentarily." $ColorWarning
		Start-Process -FilePath $helperPath -ArgumentList "`"$tempExtractDir`" `"$InstallPath`""
...
function Install-Update {
	...
	try {
		Invoke-UpdateProcess -DownloadUrl $DownloadUrl -Version $Version
	} catch {
...
function Get-AutoUpdateEnabled {
	$flagFile = Join-Path $InstallPath "auto-update.enabled"
...
function Toggle-AutoUpdate {
	...
	$flagFile = Join-Path $InstallPath "auto-update.enabled"
	$isEnabled = Test-Path $flagFile
	
	if ($isEnabled) {
		Remove-Item $flagFile -Force -ErrorAction SilentlyContinue
		Write-ColoredOutput "✖ Auto-update on startup has been DISABLED." $ColorWarning
	} else {
		# Ensure install path exists before writing the flag
		if (!(Test-Path $InstallPath)) {
			Write-ColoredOutput "ERROR: $AppName is not installed. Please install first." $ColorError
			Write-ColoredOutput ""
			Read-Host "Press Enter to return to menu"
			Show-Menu
			return
		}
		"enabled" | Out-File -FilePath $flagFile -Encoding UTF8 -Force
		Write-ColoredOutput "✔ Auto-update on startup has been ENABLED." $ColorSuccess
		Write-ColoredOutput "The app will silently check for updates each time it starts." $ColorInfo
	}
	
	# Regenerate the startup script if a startup task exists
	if (Test-AdminStartupTask) {
		Write-ColoredOutput ""
		Write-ColoredOutput "Regenerating startup script to apply the change..." $ColorInfo
		Create-IntelligentStartupScript -AppPath $AppPath -AppName $AppName | Out-Null
		Write-ColoredOutput "Startup script updated." $ColorSuccess
	}
...
function Invoke-SilentUpdate {
	# Silent update mode: no prompts, no UI. Used by the startup script.
	...
	try {
		...
		$lastCheckFile = Join-Path $InstallPath "last-update-check.txt"
		if (Test-Path $lastCheckFile) {
			$lastCheck = Get-Date (Get-Content $lastCheckFile)
			if ((Get-Date) -lt $lastCheck.AddHours(12)) {
				return $false # Checked recently, skip
			}
		}
		(Get-Date).ToString("o") | Out-File -FilePath $lastCheckFile -Encoding UTF8 -Force
		
		if (!(Test-Path $AppPath)) { return $false }
		
		$flagFile = Join-Path $InstallPath "auto-update.enabled"
		if (!(Test-Path $flagFile)) { return $false }
		
		# Query GitHub for the latest release
		$webRequest = [System.Net.WebRequest]::Create($GitHubApiUrl)
		$webRequest.UserAgent = "PowerShell-WinKeyRemapper-Updater"
		$webRequest.Accept = "application/vnd.github.v3+json"
		$webRequest.Timeout = 10000  # 10 second timeout for startup
		...
		if ([string]::IsNullOrWhiteSpace($latestVersion) -or $latestVersion -eq "release" -or $latestVersion -eq "latest") {
			return $false
		}
		
		$currentVersion = Get-ApplicationVersion -ExePath $AppPath
		if (!(Compare-Versions -CurrentVersion $currentVersion -LatestVersion $latestVersion)) {
		...
		if (-not $downloadAsset) {
			$downloadAsset = $releaseInfo.assets | Where-Object { $_.name -like "*.zip" } | Select-Object -First 1
		}
		if (-not $downloadAsset) { return $false }
		
		$downloadUrl = $downloadAsset.browser_download_url
		# Hand off to the central update pipeline
		Invoke-UpdateProcess -DownloadUrl $downloadUrl -Version $latestVersion -IsSilent
		
		# If it fails to hand off, return false
		return $false
	} catch {
		# Silent mode: swallow errors gracefully
		return $false
	}
}
...
function Create-IntelligentStartupScript {
	...
	# Build the optional silent auto-update block
	$autoUpdateBlock = ""
	if (Get-AutoUpdateEnabled) {
		$installerPath = Join-Path $InstallPath "WinKeyRemapper-Installer.ps1"
		$autoUpdateBlock = @"

REM ---- Silent Auto-Update Check ----
echo %date% %time% - Checking for silent auto-update... >> "C:\temp\winkey-startup.log"
if exist "$($InstallPath.Replace('\', '\\'))\auto-update.enabled" (
	powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "$installerPath" -Action silent-update >> "C:\temp\winkey-startup.log" 2>&1
	
	REM If the script exits with 99, an update is happening. Halt this batch file entirely!
	if %ERRORLEVEL% EQU 99 (
		echo %date% %time% - Update initiated. Handing off to temp updater. >> "C:\temp\winkey-startup.log"
		exit /b 0
	)
	
	echo %date% %time% - No updates applied. Continuing normal startup. >> "C:\temp\winkey-startup.log"
)
"@
	}
...
try {
	...
	switch ($Action) {
		...
		"silent-update" { Invoke-SilentUpdate }
		"toggle-auto-update" { Toggle-AutoUpdate }
		"menu" { Show-Menu }
	}
} catch {
